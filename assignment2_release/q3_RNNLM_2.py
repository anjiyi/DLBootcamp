...
class RNNLM_Model(LanguageModel):
  ...
  def add_projection(self, rnn_outputs):
    ...
    for rnn_step in rnn_outputs:
        rnn_step = tf.nn.dropout(rnn_step, self.dropout_placeholder)
        out = tf.matmul(rnn_step, U) + b_2
        outputs.append(out)
    print outputs
    #outputs = [tf.matmul(o, U) + b_2 for o in run_outputs[o]]
  ...
  def add_loss_op(self, output):
    ...
    """
    #y_list = tf.reshape(self.labels_placeholder, [-1, len(self.vocab)])
    y_list = tf.reshape(self.labels_placeholder, [self.config.batch_size * self.config.num_steps,-1])
    ...
    seq_loss = sequence_loss(output, y_list, ones)
    """
    ### END YOUR CODE
    return loss
