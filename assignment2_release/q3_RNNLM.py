...
class RNNLM_Model(LanguageModel):
  ...
  def add_loss_op(self, output):
    ...
    """
    ### YOUR CODE HERE
    pred = [tf.nn.softmax(output) for output in outputs]
    y_list = [tf.squeeze(x) for x in tf.split(1, num_steps, self.labels_placeholder)]
    ones = tf.ones([self.config.batch_size * self.config.num_steps])
    loss = tf.nn.seq2seq.sequence_loss(pred, y_list, ones)
    #loss = tf.nn.seq2seq.sequence_loss(output, [tf.reshape(self.labels_placeholder, [-1])], [ones])
    ### END YOUR CODE
    return loss
