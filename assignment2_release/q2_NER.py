...
class NERModel(LanguageModel):
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer"):
        ...
        b1 = xavier_initializer(b1_shape)
        window = tf.cast(window, tf.float32)
        X1 = tf.matmul(window,W) + b1
        X1 = tf.nn.dropout(X1, self.dropout_placeholder)
        X1 = tf.nn.relu(X1)
        tf.add_to_collection('W', tf.reduce_sum(W**2))
        
    with tf.variable_scope("Softmax"):
        U = xavier_initializer(u_shape)
        b2 = xavier_initializer(b2_shape)
        output = tf.matmul(X1,U) + b2
        output = tf.nn.dropout(output, self.dropout_placeholder)
        output = tf.nn.relu(output)
        tf.add_to_collection('W', tf.reduce_sum(U**2))
  ...
  def add_loss_op(self, y):
    ...
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels = self.labels_placeholder, logits = y), 0)
    loss += 0.5*self.config.l2 * tf.add_n(tf.get_collection('W'))
